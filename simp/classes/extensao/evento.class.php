...
final class evento extends evento_base {
    ...
    //
        ...
        switch ($this->get_atributo('visibilidade')) {
        case EVENTO_TODOS_GRUPOS:
            return true;
        default:
            return $usuario->possui_grupo($this->get_atributo('visibilidade'));
        }
    }
    ...
    //
        ...
        case 'data':
            if ($this->id_form == $this->id_formulario_inserir() &&
                !$this->possui_auxiliar('campo_data')) {
    ...
    //
        ...
        case $this->id_formulario_inserir():
            if ($dados->visibilidade) {
                $condicoes = condicao_sql::montar('cod_grupo', '=', $dados->visibilidade);
                if (!objeto::get_objeto('grupo')->possui_registros($condicoes)) {
                    $r = false;
    ...
    //
        ...
        case 'visibilidade':
            if ($valor) {
                $grupo = new grupo('', $valor, array('nome'));
    ...
    //
        ...
        $hoje = (($mes == (int)strftime('%m', $CFG->time)) && ($ano == strftime('%Y', $CFG->time))) ? strftime('%d', $CFG->time) : 0;
        ...
        $eventos_mes = objeto::get_objeto('evento')->consultar_varios($condicoes, true, array('data' => true));
        foreach ($eventos_mes as $evento) {
            $data_evento = $evento->get_atributo_data('data', false);
            $eventos[$data_evento['dia']][] = $evento;
        }
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            echo "    <th id=\"semana{$i}\" abbr=\"{$a}\" scope=\"col\"><acronym title=\"{$s}\">{$a}</acronym></th>\n";
        }
        echo "  </tr>\n";
        echo "</thead>\n";
        echo "<tbody>\n";
        ...
        for ($d = 1; $s < 7; $d++, $s++) {
            ...
            $bloco = '';
            if ($eh_visivel && !empty($bloco_eventos)) {
