...
/// @ METODOS DE LOGICA
    ...
    //
        ...
        // Se tem BOM (Byte Order Mark): ignora-lo
        $bytes_iniciais = substr($conteudo, 0, 3);
        $bom = chr(0xEF).chr(0xBB).chr(0xBF);
        if ($bytes_iniciais === $bom) {
            $conteudo = substr($conteudo, 3);
        }

        $entidades_xml = simplexml_load_string($conteudo);
        if (!$entidades_xml) {
            return false;
        }
