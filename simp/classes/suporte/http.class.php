...
final class http {
    ...
    //     - Bool $compactacao: flag indicando se deve compactar o documento
    //     - String $minificador: nome do minificador de codigo
    ...
    //
        ...
        if ($opcoes['minificador']) {
            if (!method_exists('minificador', $opcoes['minificador'])) {
                trigger_error('Minificador inexistente: '.$opcoes['minificador'], E_USER_WARNING);
            } else {
                minificador::preparar();
                ob_start(array('minificador', $opcoes['minificador']));
            }
        }
