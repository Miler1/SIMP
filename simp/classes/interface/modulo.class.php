...
//TODO: deveria ficar em dados_opcao
        ...
        if ($dados || (isset($dados_pagina->submodulo) && isset($_GET['op']))) {
    ...
    //
        ...
        if (isset($dados_form->ajuda)) {
            ...
            $st_campos = $extra['campos'] ? implode(', ', $extra['campos']) : '(nenhum)';
            $formato = '<'.$entidade->get_tabela().">\n";
            $formato .= '  <'.$entidade->get_classe().">\n";
            foreach ($entidade->get_atributos() as $a) {
                if ($a->chave == 'PK') { continue; }
