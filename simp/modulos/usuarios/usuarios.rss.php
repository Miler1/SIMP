...
$logs = $log->consultar_varios_iterador($condicoes, $campos, $ordem, $limite, max($total - $limite, 0));
unset($log);
...
if ($logs && $logs->size()) {
    foreach ($logs as $log) {
        $u = new usuario('', $log->cod_entidade, array('nome', 'login'));
        if ($u->existe()) {
            $data = strftime($CFG->formato_data.' - '.$CFG->formato_hora, $log->data);
            $opcoes = array(
                'link'    => array('valor' => $CFG->wwwmods.'usuarios/exibir.php?cod_usuario='.$log->cod_entidade),
